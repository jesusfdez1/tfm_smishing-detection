"""Entry point for the Multilingual (Cross-lingual) evaluation of Deep Learning models.

Loads a previously trained checkpoint from `output/dl/checkpoints`
and evaluates its performance exclusively on different language subsets
of the test split.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Multilingual Evaluation")
    p.add_argument("--data_root", type=str, default="data/processed")
    p.add_argument("--out_dir", type=str, default="output/dl/multilingual")
...
def main():
    ...
    print(f">> Loading datasets and languages...", flush=True)
    # Flexible dataset loading (fallback to privacy if enriched doesn't exist)
    ...
    # We need the exact texts used for training to recreate the split perfectly
    ...
    # Align languages exactly with the extracted labels
    base_df = metasms.df
    
    # Use the index of base_df to filter the raw df, ensuring lengths match exactly
    filtered_df = df.loc[base_df.index]
    
    # Safely extract languages
    if "language" in filtered_df.columns:
        languages = filtered_df["language"].fillna("un").tolist()
    else:
        print("!! WARNING: 'language' column not found. All mapped to 'un'.")
        languages = ["un"] * len(filtered_df)
    
    np_labels = np.array(labels)
    # Perform the exact same 80/10/10 split tracking texts, labels AND languages
    X_train, X_temp, y_train, y_temp, lang_train, lang_temp = train_test_split(
        texts, np_labels, languages, test_size=0.2, random_state=42, stratify=np_labels
    )
    X_val, X_test, y_val, y_test, lang_val, lang_test = train_test_split(
        X_temp, y_temp, lang_temp, test_size=0.5, random_state=42, stratify=y_temp
    )
    ...
    results_file = out_dir / "results_multilingual.csv"
    ...
    if results_file.exists() and results_file.stat().st_size > 0:
        try:
            ...
            for _, row in df_prev.iterrows():
                completed_pairs.add((row["model_key"], row["language"]))
            print(f">> Found existing results. Skipping {len(completed_pairs)} completed evaluations.")
        except Exception as e:
            print(f"!! Failed to read existing results file: {e}")
            
    print(f">> Starting multilingual evaluation on Test Set (N={len(test_df)})...", flush=True)
    ...
    for model_key in args.models:
        ...
        try:
            ...
            # Fallback to base model id if tokenizer is not in checkpoint
        ...
        lang_groups = test_df.groupby("language")
        for lang, group in lang_groups:
            if len(group) < 10:
                continue # Skip languages with very few test samples
                
            if (model_key, lang) in completed_pairs:
                print(f"  -> Skipping language: '{lang}' (Already completed)")
                continue
                
            print(f"  -> Testing language: '{lang}' (N={len(group)})", flush=True)
            
            ds = prepare_hf_dataset(group["text"].tolist(), group["label"].tolist(), tokenizer)
            preds_output = trainer.predict(ds)
            
            preds = np.argmax(preds_output.predictions, axis=1)
            true_labels = [LABEL2ID[lbl] for lbl in group["label"]]
    ...
    print(">> Multilingual evaluation finished.")
